...
class IOClientTests: XCTestCase {
    var scale = CGFloat(1)
    var imageURL = ""
    override func setUp() {
    ...
    func testSizeFit() {
        ...
        let imageOptURL = IOClient.constructURL(imageURL: imageURL, imageSize: imageSize, crop: false)
    ...
    func testSizeCrop() {
        ...
        let imageOptURL = IOClient.constructURL(imageURL: imageURL, imageSize: imageSize, crop: true)
        XCTAssertEqual(imageOptURL?.absoluteString, expectedURL)
    }
    ...
    func testBadURL() {
        ...
        let badURL = "$%$^%*%^*%^*%&*@34"
        let imageOptURL = IOClient.constructURL(imageURL: badURL, imageSize: imageSize, crop: false)
        XCTAssertEqual(imageOptURL?.absoluteString, nil)
    }
    
    func testImageViewFit() {
        let imageView = UIImageView(frame: CGRect(x: 0, y: 0, width: 300, height: 200))
        IOClient.constructURL(imageURL: imageURL, imageView: imageView, crop: false, completionHandler: { imageOptURL in
    ...
    func testImageViewCrop() {
        ...
        IOClient.constructURL(imageURL: imageURL, imageView: imageView, crop: true, completionHandler: { imageOptURL in
